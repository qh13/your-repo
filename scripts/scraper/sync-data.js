...
async function syncViaApi() {
  ...
  for (const video of videos) {
    try {
      ...
      console.log(`  ${result.success ? '✅' : '❌'} ${video.id}: ${result.action || result.error}`);
...
function syncViaSql() {
  ...
  const sqlFile = './data/sync-videos.sql';
  
  fs.writeFileSync(sqlFile, sqlStatements);
  console.log(`📄 生成 SQL 文件: ${sqlFile}`);
  
  try {
    execSync('cd ../worker && npx wrangler d1 execute jable-videos --remote --file=../scripts/scraper/data/sync-videos.sql', { encoding: 'utf8' });
